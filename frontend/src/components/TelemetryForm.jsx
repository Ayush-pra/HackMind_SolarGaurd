  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5" noValidate>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <div>
          ...
          >
            ...
            {plantInverters.map((inv) => <option key={inv.id} value={inv.id}>{inv.id} — {inv.model}</option>)}
          </select>
          {errors.inverter && <p className="mt-1 text-xs text-red-600">{errors.inverter}</p>}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
        ...
        <Inp label="Output (kW)" name="output" value={form.output} onChange={handleChange} cls={inputCls('output')} type="number" />
